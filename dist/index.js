...
function evaluate(rule, sources, allPaths) {
  ...
  return matchingSources.flatMap((file) => {
    if (!match.requires.every((pattern) => test(file.source, pattern)) || match.excludes?.some((pattern) => test(file.source, pattern))) return [];
...
function findDefaultEmptyDestructiveSync(rule, file) {
  ...
  for (const block of findFunctionBlocks(file.source)) {
    ...
    for (const candidate of defaults) {
      ...
      const semanticLines = [candidate.index, flow.seenIndex, flow.cleanupIndex].map((index) => file.source.slice(0, index).split(/\r?\n/).length);
      const line = file.status === "modified" ? semanticLines.find((candidateLine) => file.changedLines.has(candidateLine)) : semanticLines[0];
...
function bearerAttachment(fn, session, token, source, functions, afterIndex) {
  ...
  for (const pattern of patterns) {
    for (const match of fn.body.matchAll(pattern)) {
      ...
      const text2 = source.slice(index, boundedEnd);
      const value = authorizationHeaderValue(text2);
...
function authorizationHeaderValue(text2) {
  const key = /["']Authorization["']\s*(?:\]\s*=|:)/.exec(text2);
  if (key?.index === void 0) return void 0;
  let start = key.index + key[0].length;
  while (/\s/.test(text2[start] ?? "")) start += 1;
  let depth = 0;
  let quote = null;
  for (let index = start; index < text2.length; index += 1) {
    ...
    if (character === "'" || character === '"') quote = character;
      ...
      if (depth === 0) return text2.slice(start, index).trim();
      depth -= 1;
    } else if (character === "," && depth === 0) return text2.slice(start, index).trim();
  }
  return text2.slice(start).trim();
}
...
function enclosingPythonIfHeader(fn, index, source) {
  ...
  for (let line = before.length - 1; line >= 0; line -= 1) {
    const text2 = before[line] ?? "";
    const indent = pythonTextIndent(text2);
    if (text2.trim() === "" || indent >= childIndent || !/^if\b[^:]*:\s*$/.test(text2.trim())) continue;
    ...
    if (index < endIndex) return { index: headerIndex, endIndex, text: text2.trim() };
  }
  return void 0;
}
...
function isStaticallyDeadPythonLine(fn, index, source) {
  ...
  for (let line = before.length - 1; line >= 0; line -= 1) {
    ...
    const indent = text2.match(/^[ \t]*/)?.[0].length ?? 0;
    if (indent >= containingIndent || !text2.trimEnd().endsWith(":")) continue;
    if (/^(?:if\s+(?:False|0|None)|while\s+False)\s*:/.test(text2.trim())) return true;
...
function samePythonControlPath(fn, left, right, source) {
  const path = (index) => {
    ...
    for (let line = before.length - 1; line >= 0; line -= 1) {
      const text2 = before[line] ?? "";
      const indent = pythonTextIndent(text2);
      if (text2.trim() === "" || indent >= containingIndent || !text2.trimEnd().endsWith(":")) continue;
      if (/^(?:if|elif|else|for|while|try|except|finally|with)\b/.test(text2.trim())) {
        controls.unshift(`${indent}:${text2.trim()}`);
      }
      containingIndent = indent;
      if (containingIndent <= fn.indent) break;
    }
...
function normalizedPythonLine(source, line) {
  const text2 = source.split(/\r?\n/)[line - 1] ?? "";
  return executablePythonSource(text2).trim().replace(/\s+/g, " ");
}
...
function isInsideIndentedControl(fn, index, source, control) {
  ...
  for (let line = before.length - 1; line >= 0; line -= 1) {
    ...
    const indent = text2.match(/^[ \t]*/)?.[0].length ?? 0;
    if (indent >= containingIndent || !text2.trimEnd().endsWith(":")) continue;
    if (control.test(text2.trim())) return true;
    containingIndent = indent;
    if (containingIndent <= fn.indent) break;
  }
  return false;
...
function destructiveSyncFlow(body, candidate, offset) {
  const relative3 = candidate.index - offset;
  const after = body.slice(relative3 + candidate.text.length);
  ...
  if (seenAssignment === null || seenAssignment[1] === void 0 || seenAssignment.index === void 0) return void 0;
  ...
  if (cleanupMatch?.index === void 0) return void 0;
  const method = /\.(?:delete(?:_item|_many)?|remove|unlink)\s*\(/i.exec(cleanupMatch[0]);
  if (method?.index === void 0) return void 0;
...
function locateEligible(file, expression, anchors) {
  ...
  while ((match = re.exec(file.source)) !== null) {
    ...
    const line = file.status === "modified" && anchors !== void 0 ? eligibleSemanticAnchor(file, match[0], match.index, anchors) : file.source.slice(0, match.index).split(/\r?\n/).length;
    if (line === void 0) continue;
...
// src/index.ts
function createApp() {
  const app = new Adversary({ name: "python", version: "0.0.17", review: { maximumFindings: 8 } });
  registerRules(app);
